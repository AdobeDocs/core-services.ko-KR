...
고객 속성 소스를 만들고 데이터를 업로드합니다. 준비가 되면 데이터 소스를 활성화할 수 있습니다. 데이터 소스가 활성화되면 속성 데이터를 Analytics 및 Target과 공유합니다.
...
>[!IMPORTANT]
>
>이 기능에 액세스하려면 사용자 속성 제품 프로필 (고객 속성 - 기본 액세스) 에 사용자를 할당해야 합니다. ( **[!UICONTROL 관리]** &gt; **[!UICONTROL 관리 콘솔]** &gt; **[!UICONTROL 사용자]** &gt;). 고객 속성 그룹에 추가된 사용자의 경우 Experience Cloud 인터페이스 왼쪽에 [!UICONTROL 대상]의 [!UICONTROL 고객 속성] 메뉴 항목이 표시됩니다.
>
>솔루션 그룹 멤버십도 필요합니다.
...
1. 웹 사이트에 있는 각각의 고유한 랜딩 위치에 대해 [!DNL .csv].
   ...
   >[!NOTE]
   >
   >이 프로세스 후반부에 파일을 업로드하려면을 [!DNL .csv] 드래그하여 놓습니다. 그러나 FTP를 통해 [업로드하는](../attributes/t-upload-attributes-ftp.md#task_591C3B6733424718A62453D2F8ADF73B)경우 파일과 이름이 같은 [!DNL .fin] 파일도 필요합니다 [!DNL .csv].
...
1. 계속하기 전에 파일을 업로드하기 전에 [데이터 파일 요구 사항의](../attributes/crs-data-file.md#concept_DE908F362DF24172BFEF48E1797DAF19)중요 정보를 검토하십시오.
1. [아래 설명된 대로 고객 속성 소스를 만들고 데이터를](../attributes/t-crs-usecase.md#task_BCC327B2A0EF4A1BBB2934013AB92B78)업로드합니다.
...
>[!IMPORTANT]
>
>고객 속성 소스를 만들거나, 수정하거나 삭제할 때 ID가 새 데이터 소스와의 동기화를 시작하기 전에 최대 한 시간의 지연이 있습니다. 고객 속성 소스를 만들거나 수정하려면 Audience Manager에 관리 권한이 있어야 합니다. 관리 권한을 얻으려면 Audience Manager 고객 지원 센터 또는 컨설팅 팀에 문의하십시오.


1. 에서 [!DNL Experience Cloud]메뉴 ![](assets/menu-icon.png) 아이콘을 클릭합니다.
1. **[!UICONTROL 사람을]** 클릭한 다음 **[!UICONTROL 고객 속성을 클릭합니다]**.
...
1. [!UICONTROL 고객 속성 소스 편집] 페이지에서 다음 필드를 구성합니다.
   ...
   * **[!UICONTROL 이름:]** 데이터 속성 소스에 대한 친숙한 이름입니다. 의 경우 [!DNL Adobe Target], 속성 이름에는 공백을 포함할 수 없습니다. 공백이 있는 특성이 전달되면 [!DNL Target] 무시됩니다. 지원되지 않는 기타 문자는 다음과 같습니다. `< , >, ', "`.

   * **[!UICONTROL 설명:]** (선택 사항) 데이터 속성 소스에 대한 설명입니다.

   * **[!UICONTROL 별칭 ID:]** 는 특정 CRM 시스템과 같은 고객 속성 데이터의 소스를 나타냅니다. 고객 속성 소스 코드에 사용되는 고유한 ID입니다. ID는 고유해야 하고 공백 없는 소문자로 이루어져야 합니다. Experience Cloud UI에서 고객 속성 소스에 대한 별칭 ID 필드에 입력한 값은 구현(모바일 SDK의 다이내믹 태그 관리 또는 JavaScript를 통해 구현)에서 전달되는 값과 일치해야 합니다.
      ...
      * **Android:** 별칭 ID는 *Syncidentifiers* 의 [&quot;idtype&quot;](https://marketing.adobe.com/resources/help/en_US/mobile/android/?f=methods).

         예:

         `identifiers.put(`**`"idType"`**`, "idValue");`

         별칭 [ID 필드 및 고객 ID와 관련된 데이터 처리에 대한 자세한 내용은 여러 데이터 소스](../attributes/crs-data-file.md#section_76DEB6001C614F4DB8BCC3E5D05088CB) 활용을 참조하십시오.
   * **[!UICONTROL 파일 업로드:]** [!DNL .csv] 데이터 파일을 드래그 앤 드롭하거나 FTP를 통해 데이터를 업로드할 수 있습니다. (FTP를 사용하려면 [!DNL .fin] 파일도 필요합니다.) FTP를 통해 데이터 [업로드를 참조하십시오](../attributes/t-upload-attributes-ftp.md#task_591C3B6733424718A62453D2F8ADF73B).


      >[!IMPORTANT]
      >
      >특정 데이터 파일 요구 사항이 있습니다. 자세한 내용은 [데이터 파일 요구 사항을](../attributes/crs-data-file.md#concept_DE908F362DF24172BFEF48E1797DAF19) 참조하십시오.
   ...
   * **[!UICONTROL 고유 고객 ID:]** 이 속성 소스에 업로드한 고유 ID 수를 표시합니다.

   * **[!UICONTROL 고객 제공 ID가 Experience Cloud ID로 앨리어스됩니다.]** Experience Cloud ID에 별칭이 지정된 ID 수를 표시합니다.

   * **[!UICONTROL 별칭 수가 높은 고객 제공 ID:]** 500 개 이상의 Experience Cloud 방문자 ID가 별칭으로 지정된 고객 제공 ID 수를 표시합니다. 이러한 고객 제공 ID는 개인을 나타내기보다는 일종의 공유 로그인을 나타낼 수 있습니다. 시스템은 이러한 ID와 연결된 특성을 최근에 별칭으로 지정된 500개의 Experience Cloud 방문자 ID에 배포하며 별칭 수가 10,000개가 될 때까지 이 작업을 계속합니다. 이 수에 도달하면 시스템은 고객 제공 ID를 무효화하고 더 이상 연결된 속성을 배포하지 않습니다.
...
유효성 검사 프로세스를 사용하여 표시 이름 및 설명을 업로드된 속성(문자열, 정수, 숫자 등)에 매핑할 수 있습니다. 스키마를 업데이트하면 속성을 삭제할 수도 있습니다.

스키마 [유효성 검사를 참조하십시오](../attributes/validate-schema.md#concept_B3A01A15D04E4F998118E09B3A9B5043).
...
스키마에서 속성을 삭제하고 속성을 바꾸는 방법입니다.


1. 고객 속성 소스 [!UICONTROL 편집] 페이지에서 **[!UICONTROL 타겟]** 또는 **[!UICONTROL Analytics]** 가입 (가입 [!UICONTROL ]구성 아래) 를 제거합니다.
1. [업데이트된 필드가 있는 새 데이터 파일을](../attributes/t-crs-usecase.md#task_09DAC0F2B76141E491721C1E679AABC8)업로드합니다.
...
구독 [구성을 참조하십시오](../attributes/subscription.md#concept_ECA3C44FA6D540C89CC04BA3C49E63BF).

**속성 소스를 활성화하려면**

[! Uicontrol 사용자 [속성 소스 만들기 또는 편집] ] [!UICONTROL 페이지에서] 활성화 제목을 찾은 다음 **[!UICONTROL 활성을 클릭합니다]**.
...
<keyword>
Adobe Analytics
</keyword>마케팅 캠페인에서 데이터를 보고하고, 분석하고, 적절한 작업을 수행할 수 있습니다.

다음 예에서는 업로드한 속성에 따른 [!DNL Analytics] 세그먼트를 보여줍니다. 이 세그먼트는 최근에 실행한 제품이 Photoshop인 Photoshop Lightroom 가입자를 보여줍니다.

![](assets/08_crs_usecase.png)

Experience Cloud에 세그먼트를 게시하면 Experience Cloud 대상 및 Audience Manager에서 사용할 수 있게 됩니다.
...
Target에서는 대상을 만들 때 [방문자 프로필] 섹션에서 고객 속성을 선택할 수 있습니다. 모든 고객 속성은 목록에 접두사를 [!DNL crs.] 갖게 됩니다. 필요에 따라 이러한 특성을 다른 데이터 특성과 결합하여 대상을 구성합니다.
