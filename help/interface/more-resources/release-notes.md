---
description: 고객 속성, 대상자 및 사용자 관리와 같은 CX Enterprise 서비스에 대한 최신 기능, 릴리스 정보 및 알려진 문제에 대해 알아봅니다.
solution: Experience Cloud
title: Experience Cloud 인터페이스의 누적 릴리스 정보
...
CX Enterprise 중앙 인터페이스 구성 요소에 대한 기능, 릴리스 정보 및 알려진 문제입니다.

설명서 업데이트 목록은 [설명서 업데이트](doc-updates.md)를 참조하세요.

모든 응용 프로그램에 적용되는 릴리스 정보는 [CX Enterprise 릴리스 정보](https://experienceleague.adobe.com/docs/release-notes/experience-cloud/current.html?lang=ko-KR)를 참조하십시오.

## 2026년 4월

| 일자 | 업데이트 | 설명 |
| ------- | ------- | ------- |
| 2026년 4월 20일 | 홈 페이지 새로 고침 | `https://experience.adobe.com/home`에 홈 페이지의 새로운 모양과 느낌입니다. 이번 릴리스에서는 UI를 새로 고치고 전체 _어두운 모드_&#x200B;를 지원하며 접근성을 개선하여 모든 기능이 동일하게 유지됩니다. |

## 2026년 1월

| 일자 | 업데이트 | 설명 | 적용 가능한 제품 |
| -----------| -----------| ---------- | ---------- |
| 2026년 1월 14일 | 사용자 정의 가능한 왼쪽 탐색 메뉴 | 이제 CX Enterprise에서 애플리케이션의 왼쪽 탐색 메뉴를 맞춤화할 수 있습니다. 탐색 항목의 순서를 변경하고 가시성을 제어하여 가장 중요한 기능에 빠르게 액세스하십시오. 이 업데이트는 워크플로를 간소화하고 인지 부하를 줄여주며 전체 키보드 접근성을 지원합니다. | <ul><li>Experience Platform</li><li>Journey Optimizer</li><li>Journey Optimizer B2B 에디션</li><li>Dynamic Chat</li></ul> |

## 2025년 9월

| 일자 | 업데이트 | 설명 |
| -----------| -----------| ---------- |
| 2025년 9월 25일 금요일 | IP 액세스 목록 지원 | Admin Console에서 사용 가능하고 활성화된 IP 액세스 목록이 있는 조직의 경우 CX Enterprise은 `https://experience.adobe.com` 도메인의 응용 프로그램에 액세스하는 데 필요한 IP 제한 사항을 적용합니다. 이 업데이트는 해당 도메인을 통해 액세스하는 모든 웹 애플리케이션에 영향을 미치며, 해당 조직에 로그인하고 새 페이지를 로드할 때 확인이 발생합니다. |

## 2025년 3월

| 일자 | 업데이트 | 설명 |
| -----------| -----------| ---------- |
| 2025년 3월 6일 | 마우스 오른쪽 버튼 클릭 메뉴 옵션 수정 | 이제 CX Enterprise 헤더 탐색 탭에서 마우스 오른쪽 버튼 클릭, 브라우저 드롭다운 메뉴 기능을 사용할 수 있습니다. 이를 통해 2월에 출시된 스펙트럼 2 디자인 시스템에서 발생한 문제가 해결됩니다. |

## 2025년 2월

| 일자 | 기능 | 설명 |
| -----------| -----------| ---------- |
| 2월 13일 | 스펙트럼 2 | 헤더 표시줄과 헤더 표시줄에서 액세스한 구성 요소를 포함하는 CX Enterprise 애플리케이션 프레임뿐만 아니라 특정 애플리케이션의 왼쪽 탐색 레일이 Adobe의 최신 디자인 시스템인 스펙트럼 2로 업데이트됩니다. 이렇게 새로 고침된 설계에는 업데이트된 도상학이 포함되지만 동일한 기능이 포함됩니다. 그러나 헤더의 여러 요소는 다른 Adobe 사이트 및 애플리케이션과 일치하도록 재배치됩니다. |

## 2025년 1월

| 일자 | 기능 | 설명 |
| -----------| -----------| ---------- |
| 1월 9일 | 제품 사용 데이터 | CX Enterprise 제품 사용 데이터 환경 설정을 보다 쉽게 제어할 수 있도록 CX Enterprise [환경 설정](../features/account-preferences.md#product-usage-data) 페이지를 간소화하여 중복 옵션을 제거했습니다. 이를 통해 현재 사용자 환경 설정이 유지되었으며 [CX Enterprise 환경 설정](https://experience.adobe.com/preferences)에서 언제든지 환경 설정을 업데이트할 수 있습니다. |

## 2024년 10월 2일 목요일

| 기능 | 설명 |
| -----------| ---------- |
| 사용자 정의 가능한 홈 | CX Enterprise 랜딩 페이지에서 **[!UICONTROL Edit]**&#x200B;을(를) 클릭합니다. [!UICONTROL Edit] 모드를 사용하면 위젯 라이브러리 및 사용자 지정 배경에 액세스하여 CX Enterprise 홈 페이지를 개인화할 수 있습니다. [!UICONTROL Edit] 모드는 일괄 작업 및 레이아웃 조정을 포함하여 위젯을 이동, 크기 조정 및 관리할 수 있는 매끄럽고 직관적인 컨트롤을 제공하여 심미적이고 맞춤화된 환경을 제공합니다. |

## 2024년 9월 10일 수요일

| 기능 | 설명 |
| -----------| ---------- |
| Slack 알림 | CX Enterprise 알림을 Slack으로 보내도록 계정 환경 설정을 구성할 수 있습니다. 자세한 내용은 [환경 설정](../features/account-preferences.md) 도움말의 _Slack 알림_&#x200B;을 참조하십시오. |
...
* [!DNL CX Enterprise] **[!UICONTROL Help]** 메뉴에서 콘텐츠 검색이 [Experience League](https://experienceleague.adobe.com/ko#home) 검색 결과에서 애플리케이션별로 필터링되도록 업데이트되었습니다.

## 2022년 7월

| 기능 | 설명 |
| ------- | ------- |
| 통합 홈 - 빠른 액세스 위젯 | **더 빠른 이동:** 이제 홈 환경을 더욱 개인화하고 원하는 애플리케이션을 결정할 수 있습니다. 새 고정 기능을 사용하여 [!UICONTROL Quick Access]의 전면과 중앙에 표시되는 애플리케이션을 선택하십시오. <br>**스마트 고정 기능으로 최신 정보 수신:** 이제 새 애플리케이션을 더 쉽게 찾을 수 있습니다. 새로 할당된 응용 프로그램에는 _새로 만들기_ 배지가 표시되고 [!UICONTROL Quick Access]에 자동으로 고정됩니다. |
...
| 기능 | 설명 |
| ------- | -------|
| 검색 창을 통해 Experience Platform 및 Journey Optimizer에서 [!UICONTROL Recents]에 액세스 | 이제 사용자는 통합된 검색창을 통해 AEP 및 AJO의 모든 페이지에서 최근에 액세스된 오브젝트에 액세스할 수 있습니다. |
...
| 기능 | 설명 |
| ------- | -------|
| 바로 가기(**[!UICONTROL Recents]**)가 [CX Enterprise](https://experience.adobe.com/home) 홈에 추가됨 | 랜딩 페이지의 새로운 _최근 항목_ 헤더 아래에서 가장 최근의 Journey Optimizer 및 Experience Platform 작업에 대한 바로 가기에 액세스할 수 있습니다. 이 업데이트에는 일반 레이아웃 및 응답성 개선 사항도 포함됩니다. |
| **[!UICONTROL Sandboxes]**&#x200B;이(가) 헤더 표시줄로 이동됨 | 이제 샌드박스 표시기가 모든 Experience Platform 인터페이스 애플리케이션의 헤더에 통합되었습니다. 자세한 내용은 Experience Platform의 [샌드박스](https://experienceleague.adobe.com/docs/experience-platform/sandbox/ui/user-guide.html?lang=ko-KR)를 참조하십시오. |
...
| 기능 | 설명 |
| ------- | -------|
| [!UICONTROL Unified Recents] - 최근에 액세스한 비즈니스 개체에 대한 확장 지원 | [!UICONTROL Unified Recents]이(가) Journey Optimizer 및 Experience Platform의 추가 비즈니스 개체로 확장되었습니다. Journey Optimizer 고객은 Adobe Journey Optimizer 홈 페이지에서 최근에 액세스한 오브젝트(메시지, 여정, 세그먼트, 스키마, 데이터 세트, 데이터 소스, 이벤트, 작업, 소스 및 대상)를 찾을 수 있습니다. |

{style="table-layout:auto"}

## 2021년 7월

Journey Optimizer, Offers 및 Experience League에서 사용할 수 있도록 통합 검색을 업데이트합니다. 이전에는 이 기능을 Experience Platform 사용자만이 사용할 수 있습니다.

## 2021년 6월

| 기능 | 일자 | 설명 |
| ------- | ------- | ------- |
| Adobe Federated ID용 SSO(Single Sign-On) 지원 | 2021년 6월 17일 | Federated ID를 사용하는 경우 이메일 주소나 암호를 입력할 필요 없이 CX Enterprise에 로그인할 수 있습니다. 이 기능을 사용하려면 `#/sso:@domain`을(를) CX Enterprise URL에 추가하십시오. <br>예를 들어 소유하고 있는 도메인이 `example.com`이고 Adobe Analytics에 로그인하고자 한다고 가정하겠습니다. URL은 `https://experience.adobe.com/#/sso:@example.com/analytics`가 됩니다. |
| Experience League 검색 | 2021년 6월 1일 | Experience League 설명서 검색이 개선되었습니다. [Experience League](https://experienceleague.adobe.com/docs/?lang=ko)&#x200B;(으)로 이동하고 **[!UICONTROL Search]** 필드를 사용하여 튜토리얼, 설명서, 과정 등을 찾을 수 있습니다. |
...
| 기능 | 설명 |
| ------- | ------- |
| CX Enterprise 헤더 및 탐색 | Adobe CX Enterprise 업데이트에는 헤더의 밝은 테마 변경 기능과 CX Enterprise 헤더에서 어두운 테마로 쉽게 전환하고 사용자 아바타의 추가 환경 설정을 제어하는 링크가 포함되어 있습니다. CX Enterprise의 모든 애플리케이션이 테마를 지원하는 것은 아니지만, 이 기능은 향후의 테마 지원을 잠금 해제합니다. |
| CX Enterprise 글로벌 검색 | 이번 릴리스를 통해 CX Enterprise 글로벌 검색을 통해 [Experience League](https://experienceleague.adobe.com/ko#home) 설명서, 과정 및 튜토리얼을 검색할 수 있습니다. (현재 글로벌 검색은 Experience Platform 사용자만 사용할 수 있습니다. [!UICONTROL Platform]에 대한 전역 검색을 사용하면 세그먼트, 데이터 세트, 스키마 등과 같은 CX Enterprise의 모든 비즈니스 개체를 검색할 수 있습니다.) |
| CX Enterprise 언어 환경 설정 | 이 업데이트에는 CX Enterprise [환경 설정](https://experience.adobe.com/preferences)에서 원하는 언어를 설정하는 기능이 포함되어 있습니다. |
...
| 기능 | 설명 |
| -----------| ---------- |
| 관리 도구 - 정책 | 이 페이지에는 조직의 전체 CX Enterprise 정책 목록이 표시됩니다. 제품, 인스턴스, 사용자 및 개발자에 대한 정보를 제공합니다. 정책 목록을 검색, 정렬 및 필터링하여 요구에 맞게 표시할 수 있습니다. 자세한 내용은 [CX Enterprise 관리 도구](../administration/admin-tool-experience-cloud.md) 도움말을 참조하십시오. |

{style="table-layout:auto"}

## 2020년 4월

* CX Enterprise [!UICONTROL Feed] 페이지는 더 이상 사용되지 않습니다. (EXC-8505)
* 새 브랜딩 요소를 반영하도록 CX Enterprise 로그인 페이지가 업데이트되었습니다. (EXC-10747)

## 2020년 2월

| 기능 | 설명 |
| -----------| ---------- |
| 관리 도구 - 사용자 세부 정보 보기 | 관리자는 새로운 관리 도구에서 모든 CX Enterprise 사용자 목록과 세부 사항을 정렬 및 필터링 가능한 목록으로 볼 수 있습니다. 사용자 세부 정보에는 사용자의 제품 액세스, 역할 및 마지막으로 액세스한 정보가 포함됩니다. 자세한 내용은 [CX Enterprise 관리 도구](../administration/admin-tool-experience-cloud.md) 도움말을 참조하십시오. |
