...
1. Adobe 고객 지원 센터에서 Adobe 관리 인증서 프로그램에 대한 자사 데이터 수집을 설정하도록 요청하는 티켓을 엽니다.
...
* **웹 SDK(alloy)**: [`edgeDomain`](https://experienceleague.adobe.com/en/docs/experience-platform/web-sdk/commands/configure/edgedomain) 명령 내에서 [`configure`](https://experienceleague.adobe.com/en/docs/experience-platform/web-sdk/commands/configure/overview) 속성을 업데이트합니다.
...
예. 그러나 자체 인증서를 관리하는 경우 인증서를 갱신하고 갱신할 때마다 인증서를 Adobe에 제공해야 합니다. 이 프로세스는 보안성이 떨어지며 조직에서 제시간에 인증서 갱신을 잊는 경우 데이터 손실이 발생할 수 있습니다. Adobe에서는 특히 TLS 인증서 최대 수명 단축으로 인해 인증서를 직접 관리하는 대신 관리 인증서 프로그램을 사용하는 것이 좋습니다. 자세한 내용은 CA/Browser Forum 서버 인증서 기본 요구 사항의 [6.3.1 공개 키 보관](https://cabforum.org/working-groups/server/baseline-requirements/requirements/#632-certificate-operational-periods-and-key-pair-usage-periods)을 참조하십시오.
+++
