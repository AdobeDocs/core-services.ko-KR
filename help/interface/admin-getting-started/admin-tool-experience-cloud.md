---
description: 모든 Experience Cloud 사용자 및 정책에 대한 정렬 및 필터링 가능한 목록을 보려면 Experience Cloud 관리 도구에 대해 알아보십시오.
keywords: core services
seo-description: 모든 Experience Cloud 사용자 및 정책에 대한 정렬 및 필터링 가능한 목록을 보려면 Experience Cloud 관리 도구에 대해 알아보십시오.
...
관리자는 관리 도구에 세부 정보가 포함된 모든 Experience Cloud 사용자 및 정책의 정렬 및 필터링 가능한 목록을 볼 수 있습니다. 사용자 세부 정보에는 사용자의 제품 액세스, 역할 및 마지막으로 액세스한 정보가 포함됩니다. 정책 세부 사항에는 정책의 (제품 프로필) 사용자, 그룹, 개발자, 통합 및 관리 목록과 정책에 대한 자세한 권한 및 리소스 정보가 포함됩니다. (**참고:** 사용자 및 제품 관리는 [Admin Console](admin-getting-started.md)에서 구성됩니다.)
...
이 페이지에는 조직의 전체 Experience Cloud 정책 목록이 표시됩니다. 제품, 인스턴스, 사용자 및 개발자에 대한 정보를 제공합니다. 정책 목록의 사용자 지정 보기를 검색, 정렬 및 필터링할 수 있습니다.

| 요소 | 설명 |
|---|---|
| [!UICONTROL 제품 프로필] | 제품 프로필의 이름입니다. 열은 A->Z, Z->A로 정렬할 수 있습니다.정책에 대한 자세한 내용을 보려면 제품 프로필 이름을 클릭하십시오. |
| [!UICONTROL 제품] | 제품 프로필과 연결된 제품 열은 A->Z, Z->A로 정렬할 수 있습니다. |
| [!UICONTROL 인스턴스] | 제품 프로필과 연결된 인스턴스(예: 테넌트 또는 로그인 회사). 고유 인스턴스 또는 테넌트가 없는 제품은 값에 대해 &quot; -&quot;를 표시합니다. 열은 A->Z, Z->A로 정렬할 수 있습니다. |
| [!UICONTROL 사용자 수] | 직접 할당 및 그룹 할당을 포함하여 제품 프로필과 연결된 사용자의 고유 수입니다. 열은 가장 작게 또는 가장 큰 열과 가장 작은 것으로 정렬할 수 있습니다. |
| [!UICONTROL 개발자 수] | 제품 프로필과 연관된 개발자 역할 수입니다. 열은 가장 작게 또는 가장 큰 열과 가장 작은 것으로 정렬할 수 있습니다. |

## 정책 목록 보기 사용자 지정

열을 검색, 정렬 또는 필터링하여 정책 목록을 사용자 정의할 수 있습니다.

* 이름별로 제품 프로필을 검색합니다. 입력한 텍스트 문자열과 일치하는 항목을 찾는 검색입니다.
* 오름차순 또는 내림차순 값으로 열을 정렬합니다. 이는 제품 프로필,  제품, [!UICONTROL 인스턴스,] 사용자 수, [!UICONTROL 개발자 수,]   열 수에 적용됩니다.
* Click the **[!UICONTROL Filter By]** icon to apply multiple filters to list product profiles with specific criteria. 여러 필터 카테고리가 적용되면 검색에 연결된 인스턴스 솔루션 그룹 `AND` 이 `AND` 포함됩니다.

| 요소 | 설명 |
|---------|----------|
| [!UICONTROL 인스턴스] 필터 | 인스턴스 열에서 문자 문자열을 검색하여 하나 또는 여러 개의 인스턴스로 결과를 좁힙니다. 각 검색어 뒤에 Enter 키를 눌러 여러 필터를 추가합니다. |
| [!UICONTROL 솔루션] 필터 | 사용 가능한 솔루션 중에서 선택합니다. 여러 솔루션 필터가 솔루션 1 `OR` 솔루션 2가 포함된 결과를 검색합니다. |

## 정책 세부 사항 보기

정책  페이지에서 정책의 세부 사항을 보려면 제품 프로필 이름을 클릭합니다.

각 제품 프로필에 대한 세부 보기에는 제품 프로필의 주제(사용자, 그룹 등)에 대한 중요한 세부 정보와 제품 프로필에서 활성화한 권한 및 리소스가 표시됩니다.  제품 프로필의 세부 사항을 CSV 파일로 내보낼 수 있습니다.  CSV [!UICONTROL 내보내기] 옵션을 사용하면 다음과 같은 2개의 CSV 파일이 생성됩니다.제목 세부 사항(사용자, 사용자 그룹, 개발자, 통합, 관리자) 및 권한 및 리소스 항목.

## 요약 섹션

이 섹션에는 다음과 같은 제품 프로필의 요약이 표시됩니다.
...
| 탭 | 설명 |
|---------|----------|
| [!UICONTROL 사용자] | 제품 프로필에 포함된 사용자 목록입니다. 사용자 그룹 연결이 그룹별 [!UICONTROL 으로 지정됨 열에] 나타납니다. |
...
이 섹션에는 제품 프로필에 사용할 수 있는 권한 및 리소스의 전체 목록이 표시됩니다. 제품 프로필에 포함된 권한 및 리소스는 &quot;✔&quot;으로 표시되었습니다.  사용 권한 및 리소스 목록은 탭 및 열로 분류되어 쉽게 볼 수 있습니다.  탭과 열에는 현재 제품에 적용되는 섹션 목록이 표시됩니다.
