...
기존 고객의 경우, 고객 속성 및 대상과 같은 기능을 사용할 수 있도록 솔루션 구현을 현대화하고 Experience Cloud를 구현하는 방법을 알아봅니다. 이를 실현하려면 다음을 수행합니다.
...
1. [Experience Cloud 조직에 보고서 세트 매핑](#section_7B08516B01BA421681DF03D0E86CE3BA)
...
* **Adobe Analytics:** Standard 또는 Premium(기존 SKU가 [!DNL SiteCatalyst] 아님).
* **Adobe Target:** Standard 또는 Premium.

>[!NOTE]
>
>의 [!DNL Target]경우 at.js에서 마이그레이션합니다 [!DNL mbox.js]. See [Upgrading from at.js 1. x to at.js 2. x](https://docs.adobe.com/content/help/en/target/using/implement-target/client-side/upgrading-from-atjs-1x-to-atjs-20.html).
...
1. 계정 관리자에게 문의하여 Experience Cloud의 프로비저닝 프로세스를 시작하십시오.
...
이전에 [!UICONTROL Analytics] > 관리 도구에서 관리했던 Analytics 그룹과 같이 이미 솔루션 그룹의 구성원인 사용자가 있을 가능성이 [!UICONTROL 높습니다].

이러한 그룹을 Experience Cloud 엔터프라이즈 그룹에 매핑하면 해당 사용자는 자신의 솔루션 계정 자격 증명을 Adobe ID에 수동으로 연결해야 합니다.
...
다음 섹션에서는 구현을 현대화하는 방법을 설명합니다. 구현을 현대화하면 Experience Cloud의 핵심 서비스가 활성화됩니다.

## 2단계. Experience [!UICONTROL Platform Launch] 또는 [!UICONTROL 다이내믹 태그 관리를 사용하여 Experience Cloud ID 서비스][!UICONTROL 구현] {#section_3C9F6DF37C654D939625BB4D485E4354}

Experience [!UICONTROL Cloud ID Service] 는 솔루션 간 통합을 위한 공통 ID를 제공합니다. 고객 속성을 통해 업로드된 CRM 데이터를 기반으로 크로스 도메인 방문자 식별 및 크로스 디바이스/브라우저 타깃팅 및 개인화를 위한 경로를 [!UICONTROL 제공합니다].

Experience Cloud 핵심 서비스를 활성화하는 가장 간단한 방법은 Experience Platform Launch의 [Experience Cloud ID Service 확장](https://docs.adobe.com/content/help/ko-KR/launch/using/implement/solutions/idservice-save.html) 기능을 통해 또는 [!UICONTROL 다이내믹 태그 관리의 ECID 도구를 통해 Analytics 및 Adobe Target에 대해 자동으로 활성화하는]것입니다 . (Experience Platform Launch가 적극 권장됩니다.)

![](assets/menu-activation-shell.png)

전체 Experience Cloud ID 서비스 도움말(이전 방문자 ID)을 보려면 [여기로 이동하십시오](https://docs.adobe.com/content/help/ko-KR/id-service/using/home.html).

**경험[!UICONTROL 플랫폼 실행]또는[!UICONTROL 다이내믹 태그 관리를 사용하지 않습니까]?**
...
| 작업 | 설명 |
| -----------| ---------- |  
| [Analytics용 Experience Cloud ID 서비스 구현](https://docs.adobe.com/content/help/en/id-service/using/implementation/setup-analytics.html) | 추가 [고객 ID를 설정하는 것도 좋습니다](https://docs.adobe.com/content/help/ko-KR/id-service/using/reference/authenticated-state.html). 이러한 ID는 각 방문자와 연결되며 Experience Cloud에서 현재 및 향후 기능을 활성화합니다. |
| 기존 [!DNL s_code]를 버전 H.27.3 이상으로 업데이트하거나 기존 [!DNL AppMeasurement.js]를 버전 1.4 이상으로 업데이트합니다. | 이러한 파일은 Analytics 관리 도구의 [코드](https://docs.adobe.com/content/help/en/analytics/admin/admin-tools/code-manager-admin.html) 관리자에서 다운로드할 수 있습니다.  ( [JavaScript 구현](https://docs.adobe.com/content/help/en/analytics/implementation/javascript-implementation/javascript-implementation-overview.html) 안내서는 에 대한 자세한 내용이 필요한 경우 사용할 수 [!DNL AppMeasurement.js]있습니다.) |
...
Experience Cloud ID 서비스 설정의 일부로, Analytics에 대해 권장하며, [!DNL Target] 고객 ID를 Experience Cloud와 [](https://docs.adobe.com/content/help/ko-KR/id-service/using/reference/authenticated-state.html) 동기화하는 것이 좋습니다.

In Adobe Target, the `mbox3rdpartyid` needs to get the customer ID and send it to [!DNL Target]. (다음 [에서 고객 속성](https://docs.adobe.com/content/help/ko-KR/target/using/audiences/visitor-profiles/working-with-customer-attributes.html) 작업을 [!DNL Target]참조하십시오.)

방문자가 웹 사이트에서 인증을 받거나 다른 방법으로 식별되면 페이지 또는 앱에 해당 개인의 CRM 고객 ID가 표시되어야 합니다. 그런 다음 적절한 함수 호출을 사용하여 고객 ID를 Experience Cloud와 동기화할 수 있습니다. 이러한 동기화는 방문자의 CRM 고객 ID를 Experience Cloud에 저장하고 해당 고객의 속성이 Experience Cloud에서 사용하도록 활성화됩니다.
...
Android *및* iOS [모바일 애플리케이션에서 추가 고객 ID를 설정하는 방법에 대한 구문 예는 Experience Cloud ID 서비스](https://docs.adobe.com/content/help/ko-KR/mobile-services/android/overview.html)[섹션을](https://docs.adobe.com/content/help/ko-KR/mobile-services/ios/overview.html) 참조하십시오.

### 이전 데이터의 속성 활성화

고객 속성 데이터는 방문자가 로그인한 후에 사용할 수 있습니다. 최신 Experience Cloud ID 서비스를 아직 구현하지 않았으며, 이전에 prop 또는 eVar에서 고객 ID를 추적해온 경우, Experience Cloud에 내역 로그인을 보내는 프로세스를 요청할 수 있습니다. 이 프로세스를 통해 고객 속성을 즉시 사용할 수 있습니다.
...
Experience Cloud 서비스(예: Experience Cloud ID 서비스 및 [!UICONTROL 사람 서비스])는 개별 Analytics 보고서 세트 대신 Experience Cloud 조직과 연결됩니다. 이러한 서비스가 올바르게 작동하도록 하려면 각 Analytics 보고서 세트를 Experience Cloud 조직에 매핑해야 합니다.
...
In [!UICONTROL Dynamic Tag Management], click **[!UICONTROL <Web Property Name>]**>**[!UICONTROL &#x200B;개요&#x200B;]****[!UICONTROL > 도구&#x200B;]**추가**[!UICONTROL >]**Adobe Analytics. 배포 정보는[다이내믹 태그](https://docs.adobe.com/content/help/en/dtm/using/tools/analytics-dtm.html)관리의 Adobe Analytics 설정을 참조하십시오.

## 5단계. (Adobe Target) Update your Adobe Target implementation {#section_C2F4493C7A36406DAE2266B429A4BD24}

* 라이브러리 검색이 자동으로 이루어지도록 [Experience Platform Launch](https://docs.adobe.com/content/help/en/launch/using/extensions-ref/adobe-extension/targetv2-extension/adobe-target-extension-v2.html) 에서 [!UICONTROL Adobe Target 확장 기능을]추가하는 것이 좋습니다. Experience Platform Launch를 사용하여 [Adobe Target(및 기타 솔루션)용 Experience Cloud ID 서비스 확장](https://docs.adobe.com/content/help/ko-KR/launch/using/extensions-ref/adobe-extension/id-service-extension/overview.html) 기능을 설정할 수도 [!UICONTROL 있습니다]. Adobe [!UICONTROL Target에서 핵심 서비스를 사용하려면] Experience Cloud ID 서비스 **** 업데이트가 필요합니다. 다이내믹 태그 [!UICONTROL 관리를]사용하는 경우 [Adobe Target 도구를 추가합니다](https://docs.adobe.com/content/help/en/dtm/using/tools/target.html). 또한 [!UICONTROL 다이내믹 태그 관리를 사용하여] Adobe Target용 Experience Cloud ID 서비스를 배포할 수도 있습니다.)
* 경험 플랫폼 실행 또는 [!UICONTROL 다이내믹 태그 관리를] 사용하지 않는 경우 [!UICONTROL mbox 라이브러리를][수동으로](https://docs.adobe.com/content/help/en/target/using/implement-target/client-side/mbox-implement/target-download-config-mbox.html) 업데이트하십시오.
* Request access to use Adobe Analytics as the reporting source for [!DNL Adobe Target]. [!DNL Target] 및 데이터가 처리 중에 동일한 서버 호출에 결합되므로 방문자가 두 솔루션 간에 연결됩니다. [!DNL Analytics] See [Analytics for Target Implementation](https://docs.adobe.com/content/help/ko-KR/target/using/integrate/a4t/a4t.html).
...
다음 프로세스를 사용하여 사이트에서 Experience Cloud ID 서비스가 올바르게 구현되도록 하십시오.

1. Experience Cloud ID 서비스에 대한 요청을 볼 수 있도록 사이트에 대한 쿠키를 지웁니다. 요청은 첫 번째 방문에서 수행된 후 매주 방문자마다 거의 1번씩 수행됩니다.
...
1. Analytics 요청에 매개 변수 `mid` (Experience Cloud ID)가 포함되어 있는지 확인합니다. 유예 기간 동안(활성화된 경우) 매개 변수(Analytics 방문자 ID)도 `aid` 표시됩니다.

Experience Cloud ID가 포함된 예상 응답:

![](assets/mac_id_response.png)

Experience Cloud ID( `mid` 또는 _방문자 ID라고도 함)를 포함하는 분석 이미지 요청_:
...
Experience Cloud ID 서비스를 배포한 후 새 방문자는 더 이상 데이터 수집 서버에서 Analytics Experience Cloud ID를 받지 않습니다. 사이트의 섹션이 아직 Experience Cloud ID 서비스를 구현하지 않은 경우 방문자가 이러한 섹션으로 이동하면 Experience Cloud ID가 인식되지 않으며 방문자에게 이전 Analytics 방문자 ID가 할당됩니다. 이로 인해 중복 방문 및 잘못된 속성을 포함하여 잠재적인 문제가 발생할 수 있습니다.

예를 들어 사이트의 지원 섹션이 별도의 CMS에서 관리되는 경우 이 섹션에 대해 다른 Analytics JavaScript 파일이 있을 수 있습니다. 지원 사이트에 ID 서비스를 배포하기 전에 기본 사이트에 Experience Cloud ID를 배포하는 경우 새 방문자가 지원 섹션을 방문할 때 기존 Analytics ID를 받고, 두 사이트 섹션에 걸쳐 있는 방문은 서로 다른 방문으로 보고됩니다.

여러 JavaScript 파일 또는 기타 기술(예: Flash)을 사용하는 사이트에 Experience Cloud ID 서비스를 배포하면 동시에 사이트의 모든 부분에서 Experience Cloud ID 서비스를 활성화해야 하므로 조정 문제가 발생할 수 있습니다. 유예 기간을 구성하면 새 방문자가 ID 서비스에서 Analytics 방문자 ID를 계속 받게 되므로 방문자는 업그레이드되지 않은 사이트의 섹션에서 일관되게 식별되어 방문자 ID 서비스를 사용할 수 있습니다.
...
* Audience Manager에서 설정한 규칙에 따라 방문자가 자격을 평가하는 트레이트 및 세그먼트
* (선택 사항) 하나 이상의 ID. ID 서비스 구현에 따라 CRM ID 또는 해시된 이메일 주소와 같은 하나 이상의 ID를 전송할 수도 있습니다. 이 데이터가 Adobe [!DNL Analytics]로 전송되면 Adobe 고객 관리로 전달됩니다. 개인 데이터는 Adobe [!DNL Analytics]에 제공하지 않는 것이 좋습니다. 대신 단방향 해시를 사용하여 데이터를 Adobe로 보내기 전에 마스크를 적용합니다.
...
이러한 모든 데이터 요소는 로그 파일 형태로 Adobe Audience Manager에 전달됩니다. Audience Manager는 미국 내에서 이 데이터를 처리하고 저장합니다. Audience Manager는 미국 외부에서 이 데이터를 저장 또는 처리하는 옵션을 제공하지 않습니다.
...
도메인 간 [추적을 활성화하려면 데이터 수집 CNAME 및 도메인 간](https://docs.adobe.com/content/help/ko-KR/id-service/using/reference/analytics-reference/cname.html) 추적을 참조하십시오.
