...
[!UICONTROL Experience Platform 태그]를 사용하지 않는 경우 다음과 같이 JavaScript 배포(`VisitorAPI.js`)를 통해 방문자 ID 서비스를 수동으로 구현하십시오.
...
* 방문자 ID 서비스를 사용하여 `visitor.setCustomerIDs({"crm_id":"52mc210tr42"})`을(를) 호출합니다. 또는
...
고객 속성을 처음 사용할 수 있게 되었을 때 일부 고객은 방문자 ID 서비스를 아직 구현하지 않았으며 고객 속성을 쉽게 활용할 수 없었습니다. 이 문제를 완화하기 위해 Adobe는 Adobe Analytics Data Warehouse를 사용하여 ID 동기화 채우기를 수행하는 수단을 마련했습니다. 이 기능을 Data Warehouse 채우기라고 합니다. 이제 Data Warehouse 채우기는 일반적으로 필요하지 않으므로 2022년 10월부터 이 기능을 더 이상 사용할 수 없습니다.
...
고객 속성 데이터는 방문자가 로그인한 후에 사용할 수 있습니다. 방문자 ID 서비스를 아직 구현하지 않았으며 이전에 prop 또는 eVar에서 고객 ID를 추적해 온 경우, CX Enterprise에 내역 로그인을 전송하는 프로세스를 요청할 수 있습니다. 이 프로세스를 진행하면 고객 속성을 바로 사용할 수 있습니다.
...
다음 프로세스에 따라 사이트에서 방문자 ID 서비스가 올바르게 구현되도록 하십시오.

1. 방문자 ID 서비스에 대한 요청을 볼 수 있도록 사이트에 대한 쿠키를 지웁니다(이 요청은 첫 번째 방문에서 수행된 후 매주 방문자마다 한 번씩 수행됩니다).
...
1. Analytics 요청에 `mid` 매개 변수(ECID)가 포함되어 있는지 확인하십시오. 유예 기간(활성화된 경우) 동안 `aid` 매개변수(Analytics 방문자 ID)도 표시되어야 합니다.

ECID가 포함된 예상 응답은 다음과 같습니다.

![ECID가 포함된 예상 응답](../assets/mac_id_response.png)

ECID(`mid` 또는 _방문자 ID_)가 포함된 Analytics 이미지 요청:

![ECID가 포함된 Analytics 이미지 요청](../assets/mid.png)

mbox 요청의 ECID:

mbox 요청의 ![ECID](../assets/mbox_request.png)

### 유예 기간이란 무엇입니까?

방문자 ID 서비스를 배포하면 새 방문자는 더 이상 Analytics ID(`aid`)를 받지 않습니다. 사이트의 섹션이 방문자 ID 서비스를 아직 구현하지 않은 경우 방문자가 이러한 섹션으로 이동하면 ECID(`mid`)가 인식되지 않으며 방문자에게 이전 Analytics ID(`aid`)가 지정됩니다. 이 설정으로 인해 중복 방문 및 잘못된 속성을 포함한 잠재적 문제가 발생할 수 있습니다.

예를 들어 사이트의 지원 섹션에 별도의 CMS에서 관리되는 경우 이 섹션에 대해 다른 Analytics JavaScript 파일을 보유할 수 있습니다. 방문자 ID 서비스를 지원 사이트에 배포하기 전에 기본 사이트에 ECID를 배포하는 경우, 새 방문자가 지원 섹션을 방문할 때 기존 Analytics ID를 받게 됩니다. 두 사이트 섹션에 걸쳐 있는 방문은 서로 다른 방문으로 보고됩니다.

여러 JavaScript 파일 또는 다른 기술(예: Flash)을 사용하는 사이트에 방문자 ID 서비스를 배포하면 조정 문제가 발생할 수 있습니다. 이러한 문제는 사이트의 모든 부분에서 동시에 방문자 ID 서비스를 사용하도록 설정해야 하기 때문에 발생합니다. 유예 기간을 구성하면 새 방문자가 방문자 ID 서비스에서 Analytics 방문자 ID를 계속 받게 됩니다. 방문자는 방문자 ID 서비스를 사용하도록 업그레이드되지 않은 사이트의 섹션에서 일관되게 식별될 수 있습니다.
