...
* **그룹 구성원:** 데이터를 업로드하려면 사용자가 [!DNL Customer Attributes] 그룹의 구성원이어야 합니다. 또한 Adobe Analytics 그룹 또는 Adobe Target 그룹에 속해야 합니다.
  ...
  고객 특성에 액세스할 수 있는지 확인하려면 [!DNL CX Enterprise] 관리자가 [CX Enterprise](https://experience.adobe.com)에 로그인해야 합니다. **[!UICONTROL Admin Console]** > **[!UICONTROL Products]**(으)로 이동합니다. *[!DNL Customer Attributes]*&#x200B;이(가) [!UICONTROL product profiles] 중 하나로 표시되면 시작할 수 있습니다.
...
CX Enterprise의 _[!UICONTROL Create Customer Attribute Source]_&#x200B;페이지에서 다음 단계를 수행합니다.
...
1. [!UICONTROL Customer Attributes]을(를) 열려면 **[!UICONTROL Apps]** ![메뉴](assets/menu-icon.png) > **[!DNL Customer Attributes]**&#x200B;을(를) 클릭합니다.
...
1. [!UICONTROL Create Customer Attribute Source] 페이지에서 다음 필드를 구성합니다.
   ...
   * **[!UICONTROL Alias ID:]** 특정 CRM 시스템과 같은 고객 특성 데이터의 원본을 나타냅니다. [!UICONTROL Alias ID]은(는) [!UICONTROL customer attribute Source] 코드에서 사용되는 고유 ID입니다. ID는 고유해야 하고 공백 없는 소문자로 이루어져야 합니다. CX Enterprise에서 고객 특성 소스에 대한 [!UICONTROL Alias ID] 필드에 입력한 값은 구현(Mobile SDK의 Platform Data Collection 또는 JavaScript을 통해)에서 전달되는 값과 일치해야 합니다.
      ...
      * **태그:** 별칭 ID는 [CX Enterprise ID 서비스](https://experienceleague.adobe.com/docs/experience-platform/tags/home.html?lang=ko) 도구의 [!UICONTROL customer Settings]에 있는 *통합 코드* 값에 해당합니다.
   ...
   * **[!UICONTROL Namespace Code:]** AEP WebSDK 구현의 일부로 [IdentityMap](https://experienceleague.adobe.com/ko/docs/experience-platform/web-sdk/identity/overview)을(를) 사용할 때 이 값을 사용하여 고객 특성 소스를 식별하십시오.

1. **[!UICONTROL Save]**&#x200B;을(를) 클릭합니다.

## 파일 업로드 {#upload-customer-attributes}
...
* **[!UICONTROL Customer-Provided IDs Aliased to CX Enterprise Visitor IDs:]**&#x200B;은(는) 몇 개의 ID가 CX Enterprise 방문자 ID에 별칭이 지정되었는지 표시합니다.

* **[!UICONTROL Customer-Provided IDs with High Alias Counts:]**&#x200B;은(는) 500개 이상의 별칭 지정된 CX Enterprise 방문자 ID가 있는 고객 제공 ID 수를 표시합니다. 이러한 고객 제공 ID는 개인을 나타내기보다는 일종의 공유 로그인을 나타낼 수 있습니다. 시스템은 이러한 ID와 연결된 속성을 최근에 별칭으로 지정된 500개의 CX Enterprise 방문자 ID에 배포하며 별칭 수가 10,000개가 될 때까지 이 작업을 계속합니다. 그러면 시스템은 고객 제공 ID를 무효화하고, 연결된 속성을 더 이상 배포하지 않습니다.
...
[!DNL Target] 도움말에서 [대상자 만들기](https://experienceleague.adobe.com/docs/target/using/audiences/create-audiences/audiences.html?lang=ko-KR)를 참조하십시오.
