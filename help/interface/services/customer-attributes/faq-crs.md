...
| **(Analytics 전용)** Analytics Premium에 제공된 기능과 비교할 때 이 기능은 어떤 차이가 있습니까? | 이전에는 고객 속성 데이터를 Analytics 데이터에 결합하려는 고객이 이 기능을 사용하기 위해 Data Workbench 도구에 상당히 의존했습니다. [!DNL Customer Attributes]은(는) [!DNL Customer Attributes]을(를) Report Builder에서 차원 및 지표로 제공하여 좀 더 다양한 대상자에게 이 기능을 제공합니다. Analytics Standard 고객은 [!DNL Customer Attributes]에 액세스할 수 있지만 사용할 수 있는 기능은 제한됩니다. Analytics 프리미엄 고객은 전체 기능을 사용할 수 있습니다. |
