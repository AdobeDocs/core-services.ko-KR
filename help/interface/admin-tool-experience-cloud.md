...
1. [!UICONTROL 빠른 액세스]관리 도구에서 **[!UICONTROL 관리 도구를 선택합니다.]**
...
| 요소 | 설명 |
|---|---|
| [!UICONTROL 이름] | 사용자의 이름과 성씨를 입력합니다. 이 열을 A에서 Z로, Z에서 A로 정렬할 수 있습니다. 사용자에 대한 자세한 내용을 보려면 사용자 이름을 선택하십시오. |
...
| [!UICONTROL 마지막 로그인] | Experience Cloud에 최근 사용자가 로그인한 시간 및 날짜입니다. 이 열은 오름차순 또는 내림차순 날짜로 정렬할 수 있습니다. <br> **중요:** 2020년 1월 13일부터 사용자의 마지막 로그인 데이터는 365일 동안 보관됩니다. 이 정보는 Experience Cloud의 현재 로그인 활동을 표시하기 위한 것으로서, 2020년 1월 13일 이전에 비활성 계정에 대해 조치를 취하기 위한 권장 사항은 아닙니다. |
...
* 여러 필터를 적용하여 사용자를 특정 기준으로 나열하려면 **[!UICONTROL 필터 기준]** 아이콘을 선택합니다. 여러 필터 카테고리가 적용될 때에는 검색에 이메일 도메인 `AND` ID 유형 `AND` 솔루션이 포함됩니다.
...
[!UICONTROL 사용자] 페이지에서 사용자의 세부 정보를 보려면 사용자 이메일을 선택합니다.
...
| 요소 | 설명 |
|---|---|
| [!UICONTROL 제품 프로필] | 제품 프로필의 이름입니다. 열은 A->Z, Z->A로 정렬할 수 있습니다.정책에 대한 자세한 내용을 보려면 제품 프로필 이름을 선택합니다. |
| [!UICONTROL 제품] | 제품 프로필과 연관된 제품입니다. 열은 A->Z, Z->A로 정렬할 수 있습니다. |
| [!UICONTROL 인스턴스] | 제품 프로필과 연관된 인스턴스(예: 테넌트 또는 로그인 회사)입니다. 고유 인스턴스 또는 테넌트가 없는 제품은 값에 대해 &quot; - &quot;을 표시합니다. 열은 A->Z, Z->A로 정렬할 수 있습니다. |
...
* 여러 필터를 적용하여 제품 프로필을 특정 기준으로 나열하려면 **[!UICONTROL 필터 기준]** 아이콘을 선택합니다. 여러 필터 카테고리가 적용되면 검색에 그룹 연관 `AND` 인스턴스 `AND` 솔루션이 포함됩니다.
