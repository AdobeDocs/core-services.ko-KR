...
대부분의 브라우저 및 안티스파이웨어 애플리케이션은 타사 쿠키를 거부하고 삭제하도록 설계되었습니다. Adobe은 타사 쿠키가 차단되는 경우에도 쿠키를 항상 설정할 수 있도록 해줍니다. 특정 동작은 ECID 서비스(Experience Platform ID 서비스) 또는 Analytics의 기존 식별자(s_vi 쿠키라고도 함)를 사용하는지에 따라 다릅니다.

* [Experience Platform ID 서비스(ECID 서비스)](https://experienceleague.adobe.com/docs/id-service/using/intro/overview.html?lang=ko-KR)는 수집 도메인이 사이트 도메인과 일치하는지 여부에 관계없이 자동으로 자사 쿠키를 설정합니다. 일치하지 않는 경우 Identity 서비스는 JavaScript를 사용하여 사이트의 도메인에 쿠키를 설정합니다.
* [Analytics 기존 식별자](https://experienceleague.adobe.com/docs/core-services/interface/administration/ec-cookies/cookies-analytics.html?lang=en)(`s_vi` 쿠키라고도 함)를 사용하는 경우 데이터 수집 서버를 구성한 방법에 따라 달라집니다. 데이터 수집 서버가 사이트의 도메인과 일치하면, 쿠키는 자사 쿠키로 설정됩니다. 데이터 수집 서버가 사이트의 도메인과 일치하지 않으면, 쿠키는 서드파티 쿠키로 설정됩니다. 이 경우 타사 쿠키가 차단되면 Analytics는 표준 &quot;s_vi&quot; 쿠키 대신 자사 [폴백 ID(s_fid)](cookies-analytics.md)를 설정합니다.

수집 서버가 사이트의 도메인과 일치하는지 확인하려는 경우 CNAME 구현에 지정된 사용자 지정 도메인에서 Adobe의 수집 서버로 전달을 활성화하는 CNAME 구현을 사용할 수 있습니다. 여기에는 호스팅된 Adobe 도메인을 가리키도록 CNAME 별칭을 구성하기 위한 회사의 DNS 설정 변경이 포함됩니다. 다양한 Adobe 제품이 CNAME 사용을 지원하며, 모든 경우에 CNAME은 특정 고객에 대해 신뢰할 수 있는 자사 엔드포인트를 만드는 데 사용되고, 해당 고객이 소유합니다. 여러 도메인을 제어하는 경우 도메인 간 사용자를 추적하기 위해 단일 CNAME 엔드포인트를 사용할 수 있지만, 사이트 도메인이 CNAME 도메인 쿠키와 일치하지 않으면 모두 서드파티로 설정됩니다.

>[!NOTE]
>
>컬렉션 도메인이 사이트 도메인과 일치하는지 여부에 관계없이 Apple의 ITP(Intelligent Tracking Prevention) 프로그램은 macOS의 Safari와 iOS와 iPadOS의 모든 브라우저를 포함하는 ITP가 관리하는 브라우저에서 Adobe가 설정한 자사 쿠키를 단기간 동안 만듭니다. 2020년 11월 현재, CNAME을 통해 설정된 쿠키도 JavaScript를 통해 설정된 쿠키와 동일한 만료 기간을 갖습니다. 이 만료일은 변경될 수 있습니다.

데이터 수집을 위한 CNAME을 설정하려고 하며 사이트에 HTTPS 프로토콜을 사용하는 보안 페이지가 있는 경우 Adobe을 사용하여 SSL 인증서를 얻을 수 있습니다.
...
Adobe 관리 인증서 프로그램은 Adobe 수집 서버가 사이트 도메인과 일치하도록 하는 CNAME 구현에 필요한 자사 SSL 인증서를 설정하는 데 권장되는 프로세스입니다.
...
>[!NOTE]
> 과거에는 Adobe에서 고객에게 HTTPS용 CNAME과 HTTP용 CNAME 두 가지의 설정을 권장했습니다. 트래픽을 암호화하는 것이 모범 사례이고 대개의 브라우저가 HTTP에 매우 부정적이기 때문에 더 이상 HTTP용 CNAME의 설정을 권장하지 않습니다. 필요한 경우 다음과 같이 표시됩니다.
...
자사 데이터 수집을 사용하기 위해 사이트에서 코드를 편집하려면 먼저 다음 전제 조건을 완료하십시오.
