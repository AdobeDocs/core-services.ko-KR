...
대부분의 브라우저 및 안티스파이웨어 애플리케이션은 [!DNL Analytics] 데이터 수집에서 사용되는 쿠키를 포함하여 서드파티 쿠키를 거부하고 삭제하도록 설계되었습니다. 방문자가 웹 사이트와 상호 작용하는 방식에 대한 추적을 지원하려면 자사 쿠키를 사용하도록 데이터 수집을 다음과 같이 구성해야 합니다.

자사 쿠키를 구현하는 데 두 가지 옵션을 사용할 수 있습니다.

* Experience Platform ID 서비스(ECID 서비스)를 사용하는 경우, JavaScript를 사용하여 자사 컨텍스트에서 쿠키를 자동으로 설정합니다.
* [!DNL Analytics] 레거시 식별자(`s_vi` 쿠키)를 사용하는 경우, 이는 데이터 수집 서버를 구성한 방식에 따라 다릅니다. 데이터 수집 서버가 사이트의 도메인과 일치하면, 쿠키는 자사 쿠키로 설정됩니다. 데이터 수집 서버가 사이트의 도메인과 일치하지 않으면, 쿠키는 서드파티 쿠키로 설정됩니다. 이 경우, 서드파티 쿠키가 차단되면 [!DNL Analytics]는 표준 &quot;s_vi&quot; 쿠키 대신에 자사 [대체 ID(s_fid)](cookies-analytics.md)를 설정합니다.

수집 서버가 사이트의 도메인과 일치하는지 확인하기 위해, 자사 컨텍스트에서 쿠키를 설정할 수 있게 하는 CNAME 구현을 사용해도 됩니다. 여기에는 Adobe 호스트 도메인을 가리키도록 CNAME 별칭을 구성하기 위한 회사 DNS 설정의 변경도 관련되어 있습니다. 다양한 Adobe 제품이 CNAME 사용을 지원하며, 모든 경우에 CNAME은 특정 고객에 대해 신뢰할 수 있는 자사 엔드포인트를 만드는 데 사용되고, 해당 고객이 소유합니다. 여러 도메인을 제어하는 경우 도메인 간 사용자를 추적하기 위해 단일 CNAME 엔드포인트를 사용할 수 있지만, 사이트 도메인이 CNAME 도메인 쿠키와 일치하지 않으면 모두 서드파티로 설정됩니다.

>[!NOTE]
>
>두 가지 옵션 모두, 애플의 ITP(Intelligent Tracking Prevention) 프로그램이 ITP가 관리하는 브라우저에서 자사 쿠키를 단기간 사용할 수 있게 하며, 여기에는 macOS의 Safari와 iOS와 iPadOS의 모든 브라우저가 포함됩니다. 2020년 11월 현재 두 종류의 쿠키는 모두 7일의 유효 기간으로 만료됩니다. 이 만료일은 변경될 수 있습니다.
...
자사 쿠키를 사용하도록 사이트에서 코드를 편집하려면 먼저 다음 전제 조건을 완료하십시오.
...
1. 장기간 구현에서 자사 쿠키로 이동하거나 다른 자사 컬렉션 호스트 이름으로 변경하려는 경우, Adobe는 이전 도메인에서 새 도메인으로 방문자를 마이그레이션할 것을 권장합니다.
