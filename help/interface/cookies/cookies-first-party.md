...
Analytics는 쿠키를 사용하여 이미지 요청과 브라우저 세션 간에 지속되지 않는 변수 및 구성 요소에 대한 정보를 제공합니다. Adobe에서 호스팅하는 도메인에서 오는 이러한 해로운 쿠키를 서드 파티 쿠키라고 합니다.
...
| **이 경우 추가 비용이 발생합니까?** | 아니요, Adobe는 현재 모든 Analytics 고객에게 추가 비용 없이 이 서비스를 제공하고 있습니다. |
...
>[!Note:] Experience Cloud 방문자 ID 서비스는 자사 쿠키를 활성화하기 위해 CNAME을 구성하는 대안을 제공합니다.
...
>[!Note:] `https:// protocol`을(를) 사용하는 경우 ping은 FPC 전문가가 지정한 업로드 날짜 이후에만 응답합니다. 뿐만 아니라, 보안 호스트 이름 및 비보안 호스트 이름을 ping하여 구현을 업데이트하기 전에 둘 다 올바르게 작동하도록 하십시오.
