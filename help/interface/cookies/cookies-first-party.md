...
2. CNAME 레코드를 만듭니다(아래 지침 참조).
   ...
   표를 받으면 고객 지원 담당자가 CNAME 기록을 제공해야 합니다. Adobe가 귀하를 대신하여 인증서를 구입할 수 있으려면 먼저 회사의 DNS 서버에서 이러한 레코드를 구성해야 합니다. CNAME은 다음과 유사합니다.

   **보안** - 예를 들어 호스트 이름 `smetrics.example.com`은 `example.com.adobedc.net`을 가리킵니다.

>[!NOTE]
> 이전에는 HTTPS용 CNAME 2개와 HTTP용 CNAME 1개를 설정하는 것이 좋습니다. 트래픽을 암호화하는 가장 좋은 방법이며 대부분의 브라우저에서 HTTP를 강력히 권장하기 때문에 HTTP용 CNAME을 설정하는 것이 더 이상 권장되지 않습니다. 필요한 경우 다음과 같습니다.
>    **비보안** — 호스트 이름 `metrics.example.com`은 다음을 가리킵니다.`example.com.adobedc.net`.

1. CNAME이 적절한 경우 Adobe은 DigiCert와 함께 Adobe의 프로덕션 서버에 인증서를 구입하여 설치합니다.
...
>[!NOTE]
> 여전히 비보안 기능을 사용하는 경우에는 이 모양이 다음과 같습니다.
