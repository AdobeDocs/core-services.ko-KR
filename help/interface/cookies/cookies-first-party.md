...
대부분의 브라우저 및 안티스파이웨어 애플리케이션은 Analytics 데이터 수집에서 사용되는 쿠키를 포함하여 타사 쿠키를 거부하고 삭제하도록 설계되었습니다. 방문자가 웹 사이트와 상호 작용하는 방법에 대한 추적을 지원하려면 자사 쿠키를 사용하도록 데이터 수집을 구성했는지 확인해야 합니다.

자사 쿠키를 구현하는 데 두 가지 옵션을 사용할 수 있습니다.

* Experience Platform ID 서비스(ECID 서비스라고도 함)를 사용하는 경우 JavaScript를 사용하여 자사 컨텍스트에서 쿠키를 자동으로 설정합니다.
* Analytics 기존 식별자(&quot;s_vi&quot; 쿠키라고도 함)를 사용하는 경우 데이터 수집 서버를 구성한 방법에 따라 달라집니다. 데이터 수집 서버가 사이트의 도메인과 일치하는 경우 쿠키는 자사 쿠키로 설정됩니다. 수집 서버가 현재 도메인과 일치하지 않으면 쿠키가 타사 쿠키로 설정됩니다. 이 경우 타사 쿠키가 차단되면 Analytics에서는 표준 &quot;s_vi&quot; 쿠키 대신 자사 [폴백 ID(&quot;s_fid&quot;)](https://experienceleague.adobe.com/docs/core-services/interface/ec-cookies/cookies-analytics.html?lang=en#section-65e33f9bfc264959ac1513e2f4b10ac7)를 설정합니다.

수집 서버가 사이트의 도메인과 일치하는지 확인하기 위해 자사 컨텍스트에서 쿠키를 설정할 수 있는 CNAME 구현을 사용할 수 있습니다. 여기에는 호스팅된 Adobe 도메인을 가리키도록 CNAME 별칭을 구성하기 위한 회사의 DNS 설정 변경이 포함됩니다. 다양한 Adobe 제품이 CNAME 사용을 지원하며 모든 경우에 CNAME은 특정 고객에 대해 신뢰할 수 있는 자사 엔드포인트를 만드는 데 사용되며 해당 고객이 소유합니다. 여러 도메인을 제어하는 경우 이 도메인은 단일 CNAME 엔드포인트를 사용하여 도메인에서 사용자를 추적할 수 있지만 사이트 도메인이 CNAME 도메인 쿠키와 일치하지 않는 경우에는 타사로 설정됩니다.
...
1. [자타 쿠키 요청 양식](/help/interface/cookies/assets/First_Part_Domain_Request_Form.xlsx)을 작성하고 Adobe 관리 프로그램에서 자사 쿠키를 설정하도록 요청하는 고객 지원 센터를 통해 티켓을 엽니다. 문서 내에 각 필드가 예와 함께 설명되어 있습니다.
...
>[!NOTE]
> 이전에는 고객이 HTTPS용 CNAME 1개와 HTTP용 CNAME 1개를 설정하도록 권장했습니다. 이 방법은 트래픽을 암호화하는 가장 좋은 방법이며 대부분의 브라우저에서 HTTP를 강력하게 비활성화하므로 더 이상 HTTP용 CNAME을 설정하지 않는 것이 좋습니다. 필요한 경우 다음과 같습니다.
>    **비보안** — 호스트 이름 `metrics.example.com`은(는) 다음을 가리킵니다.`example.com.adobedc.net`

1. CNAME이 제 위치에 있으면 Adobe은 DigiCert와 협력하여 Adobe의 프로덕션 서버에 인증서를 구매하고 설치합니다.
